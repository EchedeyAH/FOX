/*
 * Copia EXACTA del codigo proporcionado por el usuario
 * Adaptado minimamente para compilar como C++ en el proyecto
 */
...
// Importante: ixpci.h es un header C
extern "C" {
    #include "ixpci.h"
}
 ...
 *
 *   0x00  Bipolar  ±5 V    --> usado para corrientes ±4V
 *   0x09  Unipolar  0~5V   --> usado para posiciones 0~5V
 *
 * Config word = 0x8400 + (codigo << 6) + canal
 * -------------------------------------------------------------- */
...
typedef struct {
    ...
    char  name[32];      /* nombre de la señal          */
} channel_def_t;
...
{
    ixpci_reg_t reg;
    memset(&reg, 0, sizeof(reg)); // FIX: Inicializacion a cero
...
{
    ixpci_reg_t reg, rad;
    memset(&reg, 0, sizeof(reg)); // FIX: Inicializacion
    memset(&rad, 0, sizeof(rad)); // FIX: Inicializacion
    struct timespec t0, tn;
...
/* ---------------------------------------------------------------
 * compute_voltage
 *   Unipolar: V = raw * Vmax / 4095
 *   Bipolar:  V = (raw - 2048) / 2048.0 * Vmax
 * -------------------------------------------------------------- */
...
int main(void)
{
    ...
    printf("Test PEX-1202L Analog Inputs (Reference Implementation)\n");
