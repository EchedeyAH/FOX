...
MODULE_DESCRIPTION("ICPDAS PCI-series deiver, PCI-826 service module");
MODULE_LICENSE(ICPDAS_LICENSE);
#endif

// EXPORT_NO_SYMBOLS; /* do not export symbols */
/*#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
EXPORT_SYMBOL_GPL(ixpci826_ioctl);
EXPORT_SYMBOL_GPL(ixpci826_release);
EXPORT_SYMBOL_GPL(ixpci826_open);
#else
EXPORT_SYMBOL_NOVERS(ixpci826_ioctl);
EXPORT_SYMBOL_NOVERS(ixpci826_release);
EXPORT_SYMBOL_NOVERS(ixpci826_open);
#endif
*/
...
#endif
        open:ixpci826_open,
        release:ixpci826_release,

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
        unlocked_ioctl:ixpci826_ioctl,
#else
        ioctl:ixpci826_ioctl,
#endif
};
...
int ixpci826_open(struct inode *inode, struct file *file)
{
	/* (export)
 	 *
 	 * This function is called by ixpci.o whenever a process attempts to 
 	 * open the device file of PCI-826
 	 *
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
long ixpci826_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#else
int ixpci826_ioctl(struct inode *inode, struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#endif
{
	/* (export)
 	 *
...
int ixpci826_release(struct inode *inode, struct file *file)
{
	/* (expoet)
 	 *
 	 * This function is called by ixpci.o whenever a process attempts to
...
int init_module()
{
	...
	/* align to first PCI-826 in ixpci list  */
	for(dev = ixpci_dev; dev && dev->id != PCI_826; dev = dev->next);
