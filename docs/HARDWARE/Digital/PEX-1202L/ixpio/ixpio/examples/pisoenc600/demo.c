...
unsigned char x4_index;
unsigned char x5_index;
unsigned char x6_index;
int dev_file = 1;
  ...
  ENC6_RESET_ENCODER(CARD1,X4_axis);
  ENC6_RESET_ENCODER(CARD1,X5_axis);
  ENC6_RESET_ENCODER(CARD1,X6_axis);
  ...
  do
  {
      ...
      x4_value = ENC6_GET_ENCODER(CARD1,X4_axis);
      x5_value = ENC6_GET_ENCODER(CARD1,X5_axis);
      x6_value = ENC6_GET_ENCODER(CARD1,X6_axis);

      x1_index = ENC6_GET_INDEX(CARD1,X1_axis);
      x2_index = ENC6_GET_INDEX(CARD1,X2_axis);
      x3_index = ENC6_GET_INDEX(CARD1,X3_axis);
      x4_index = ENC6_GET_INDEX(CARD1,X4_axis);
      x5_index = ENC6_GET_INDEX(CARD1,X5_axis);
      x6_index = ENC6_GET_INDEX(CARD1,X6_axis);
      ...
      gotoxy(15,12);
      printf("  X4 : %12ld   HR,INDEX:%x\n", x4_value, x4_index);
      gotoxy(15,13);
      printf("  X5 : %12ld   HR,INDEX:%x\n", x5_value, x5_index);
      gotoxy(15,14);
      printf("  X6 : %12ld   HR,INDEX:%x\n", x6_value, x6_index);
      delay(100);
  } while (getchar()!=27);	
  closeDev();
