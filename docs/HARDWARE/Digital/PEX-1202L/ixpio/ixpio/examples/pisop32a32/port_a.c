/* Example of I/O port for PISO-P32A32.
...
int main()
{
	...
	while (getchar() != 27)
	{
		if (PIODA_Digital_Output(fd, PISOP32A32_DIO_ALL, do_data)) return FAILURE;

                sleep(1); /* 5ms for relay operating time */

                if (PIODA_Digital_Input(fd, PISOP32A32_DIO_ALL, &di_data)) return FAILURE;
