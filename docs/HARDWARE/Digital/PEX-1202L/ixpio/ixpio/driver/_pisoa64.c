...
MODULE_DESCRIPTION("ICPDAS PIO-series driver, PISO-A64 service module");
...
/*
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
EXPORT_SYMBOL_GPL(ixpisoa64_ioctl);
EXPORT_SYMBOL_GPL(ixpisoa64_release);
EXPORT_SYMBOL_GPL(ixpisoa64_open);
#else
EXPORT_SYMBOL_NOVERS(ixpisoa64_ioctl);
EXPORT_SYMBOL_NOVERS(ixpisoa64_release);
EXPORT_SYMBOL_NOVERS(ixpisoa64_open);
#endif
*/
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
long ixpisoa64_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#else
int ixpisoa64_ioctl(struct inode *inode, struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#endif
{
	/* (export)
	 *
...
int ixpisoa64_release(struct inode *inode, struct file *file)
#else
void ixpisoa64_release(struct inode *inode, struct file *file)
#endif
{
	/* (export)
	 *
...
#endif
	open:ixpisoa64_open,
	release:ixpisoa64_release,

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
        unlocked_ioctl:ixpisoa64_ioctl
#else
	ioctl:ixpisoa64_ioctl
#endif
};

int ixpisoa64_open(struct inode *inode, struct file *file)
{
	/* (export)
	 *
	 * This function is called by ixpio.o whenever a process attempts to open
	 * the device file of PISO-A64
	 *
...
int init_module()
{
	...
	/* align to first PISO-A64 in ixpio list */
	for (dev = ixpio_dev; dev && (dev->csid != PISO_A64)&&(dev->csid != PISO_A64_T3A)&&(dev->csid != PISO_A64_T3B);
		 dev = dev->next);

	if ((dev->csid != PISO_A64)&&(dev->csid != PISO_A64_T3A)&&(dev->csid != PISO_A64_T3B)) {
		printk("fail!\n");
		return FAILURE;
	}
