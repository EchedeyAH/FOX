...
/* File level history (record changes for this file here.)
   ...
   v 0.3.0  23 Mar 2011 by Golden Wang
     Give support to linux kernel 2.6.37.

   v 0.2.1  23 Jul 2007 by Golden Wang
     Give support to PISO-725 v3.1.
   ...
   v 0.0.1  7 Nov 2002 by Reed Lai
     ...
     Corrects the ipcr non-invert value.
...
MODULE_DESCRIPTION("ICPDAS PIO-series driver, PISO-725 service module");
...
/*
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
EXPORT_SYMBOL_GPL(ixpiso725_ioctl);
EXPORT_SYMBOL_GPL(ixpiso725_release);
EXPORT_SYMBOL_GPL(ixpiso725_open);
#else
EXPORT_SYMBOL_NOVERS(ixpiso725_ioctl);
EXPORT_SYMBOL_NOVERS(ixpiso725_release);
EXPORT_SYMBOL_NOVERS(ixpiso725_open);
#endif
*/
...
unsigned int eoi(ixpio_devinfo_t * dp)
{
	 ...
	 * interrupt source (INT_CHAN_0-7) from APSR (Aux Pin Status Register,)
...
#endif
		break;
	case IXPIO_DO:				/* Read back DO0 ~ DO7 */
#if DO_MASK == 0xff
		reg->value = inb(base + DO);
#else
		reg->value = inb(base + DO) & DO_MASK;
#endif
		break;
	case IXPIO_DI:				/* DI0 ~ DI7 */
#if DI_MASK == 0xff
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
long ixpiso725_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#else
int ixpiso725_ioctl(struct inode *inode, struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#endif
{
	/* (export)
	 *
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,2,0)
int ixpiso725_release(struct inode *inode, struct file *file)
#else
void ixpiso725_release(struct inode *inode, struct file *file)
#endif
{
	/* (export)
	 *
...
#endif
	open:ixpiso725_open,
	release:ixpiso725_release,

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
        unlocked_ioctl:ixpiso725_ioctl
#else
	ioctl:ixpiso725_ioctl
#endif
};
...
int ixpiso725_open(struct inode *inode, struct file *file)
{
	/* (export)
	 *
	 * This function is called by ixpio.o whenever a process attempts to open
	 * the device file of PISO-725
	 *
...
int init_module()
{
	...
	/* align to first PISO-725 in ixpio list */
	for (dev = ixpio_dev; dev && (dev->csid != PISO_725)&&(dev->csid != PISO_725_T3A)&&(dev->csid != PISO_725_T3B) && (dev->csid != PISO_725_T3C); dev = dev->next);

	if ((dev->csid != PISO_725)&&(dev->csid != PISO_725_T3A)&&(dev->csid != PISO_725_T3B) && (dev->csid != PISO_725_T3C)) {
		printk("fail!\n");
		return FAILURE;
	}
