...
MODULE_DESCRIPTION("ICPDAS PIO-series driver, PIO-D168 service module");
...
/*
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
EXPORT_SYMBOL_GPL(ixpiod168_ioctl);
EXPORT_SYMBOL_GPL(ixpiod168_release);
EXPORT_SYMBOL_GPL(ixpiod168_open);
#else
EXPORT_SYMBOL_NOVERS(ixpiod168_ioctl);
EXPORT_SYMBOL_NOVERS(ixpiod168_release);
EXPORT_SYMBOL_NOVERS(ixpiod168_open);
#endif
*/
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
long ixpiod168_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#else
int ixpiod168_ioctl(struct inode *inode, struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#endif
{
	/* (export)
	 *
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,2,0)
int ixpiod168_release(struct inode *inode, struct file *file)
#else
void ixpiod168_release(struct inode *inode, struct file *file)
#endif
{
	/* (export)
	 *
...
#endif
	open:ixpiod168_open,
	release:ixpiod168_release,

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
        unlocked_ioctl:ixpiod168_ioctl
#else
	ioctl:ixpiod168_ioctl
#endif
};
...
int ixpiod168_open(struct inode *inode, struct file *file)
{
	/* (export)
	 *
	 * This function is called by ixpio.o whenever a process attempts to open
	 * the device file of PIO-D168
	 *
...
int init_module()
{
	...
	/* align to first PIO-D168 in ixpio list */
	for (dev = ixpio_dev; dev && (dev->csid != PIO_D168)&&(dev->csid != PIO_D168_T3A)&&(dev->csid != PIO_D168_T3B); dev = dev->next);

	if ((dev->csid != PIO_D168)&&(dev->csid != PIO_D168_T3A)&&(dev->csid != PIO_D168_T3B)) {
		printk("fail!\n");
		return FAILURE;
	}
