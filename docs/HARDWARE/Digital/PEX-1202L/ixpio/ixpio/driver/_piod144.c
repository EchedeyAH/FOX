...
MODULE_DESCRIPTION("ICPDAS PIO-series driver, PIO-D144 service module");
...
/*
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
EXPORT_SYMBOL_GPL(ixpiod144_ioctl);
EXPORT_SYMBOL_GPL(ixpiod144_release);
EXPORT_SYMBOL_GPL(ixpiod144_open);
#else
EXPORT_SYMBOL_NOVERS(ixpiod144_ioctl);
EXPORT_SYMBOL_NOVERS(ixpiod144_release);
EXPORT_SYMBOL_NOVERS(ixpiod144_open);
#endif
*/
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,2,0)
int ixpiod144_release(struct inode *inode, struct file *file)
#else
void ixpiod144_release(struct inode *inode, struct file *file)
#endif
{
	/* (export)
	 *
...
#endif
	open:ixpiod144_open,
	release:ixpiod144_release,

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
        unlocked_ioctl:ixpiod144_ioctl
#else
	ioctl:ixpiod144_ioctl
#endif
};
...
# if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0)
	...
	/* enable this board */
	outb(BOARD_DIO_ENABLE, dp->base + RCR);		//0x0

	return SUCCESS;
}

int ixpiod144_open(struct inode *inode, struct file *file)
{
	/* (export)
	 *
	 * This function is called by ixpio.o whenever a process attempts to open
	 * the device file of PIO-D144
	 *
...
int init_module()
{
	...
	/* align to first PIO-D144 in ixpio list */
	for (dev = ixpio_dev; dev && (dev->csid != PIO_D144)&&(dev->csid != PIO_D144_T3A)&&(dev->csid != PIO_D144_T3B); dev = dev->next);

	if ((dev->csid != PIO_D144)&&(dev->csid != PIO_D144_T3A)&&(dev->csid != PIO_D144_T3B)) {
		printk("fail!\n");
		return FAILURE;
	}
