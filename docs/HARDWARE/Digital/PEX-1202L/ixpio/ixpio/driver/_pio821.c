...
MODULE_DESCRIPTION("ICPDAS PIO-series driver, PIO-821 service module");
...
/*
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
EXPORT_SYMBOL_GPL(ixpio821_ioctl);
EXPORT_SYMBOL_GPL(ixpio821_release);
EXPORT_SYMBOL_GPL(ixpio821_open);
#else
EXPORT_SYMBOL_NOVERS(ixpio821_ioctl);
EXPORT_SYMBOL_NOVERS(ixpio821_release);
EXPORT_SYMBOL_NOVERS(ixpio821_open);
#endif
*/
...
__inline__ unsigned int eoi(ixpio_devinfo_t * dp)
{
	 ...
	 * interrupt source (PC0/1/2/3) from ASR (Aux Pin Status Register,)
	...
	apsr = inb(dp->base + ASR) & ASR_INT_MASK;
...
#endif
		break;
	case IXPIO_ASR:		/* AUX Pin Status Register */
#if ASR_MASK == 0xff
		outb(((unsigned char) reg->value), base + ASR);
#else
		outb(((unsigned char) reg->value) & ASR_MASK, base + ASR);
#endif
		break;
...
#endif
		break;
	case IXPIO_ADGCR:		/* A/D Gain Control & multiplexer Control register */
#if ADGCR_MASK == 0xff
		outb(((unsigned char) reg->value), base + ADGCR);
#else
		outb(((unsigned char) reg->value) & ADGCR_MASK, base + ADGCR);
#endif
		break;
	case IXPIO_ADMCR:		/* A/D Mode Control Register */
#if ADMCR_MASK == 0xff
		outb(((unsigned char) reg->value), base + ADMCR);
#else
		outb(((unsigned char) reg->value) & ADMCR_MASK, base + ADMCR);
#endif
		break;
	case IXPIO_ADTCR:		/* A/D Software Trigger Control register */
#if ADTCR_MASK == 0xff
...
__inline__ int read_reg(ixpio_reg_t * reg, unsigned long int base)
{
	...
	case IXPIO_ASR:			/* AUX Pin Status Register */
#if ASR_MASK == 0xff
		reg->value = inb(base + ASR);
#else
		reg->value = inb(base + ASR) & ASR_MASK;
#endif
		break;
	case IXPIO_IPCR:		/* INT Polarity Control Register */
#if IPCR_MASK == 0xff
...
#endif
		break;
	case IXPIO_ADS:			/* A/D Statuc */
#if ADS_MASK == 0xff
		reg->value = inb(base + ADS);
#else
		reg->value = inb(base + ADS) & ADS_MASK;
#endif
		break;

	case IXPIO_ADL:			/* A/D Low Byte */
#if ADL_MASK == 0xff
	...
	case IXPIO_AD:
		...
		lbyte = inb(base + ADL);
#else
		lbyte = inb(base + ADL) & ADL_MASK;
#endif
#if ADH_MASK == 0xff
		hbyte = inb(base + ADH);
#else
		hbyte = inb(base + ADH) & ADH_MASK;
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
long ixpio821_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#else
int ixpio821_ioctl(struct inode *inode, struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#endif
{
	/* (export)
	 *
	...
	case IXPIO_DIGITAL_IN:
		...
		digital.data.u16 = digital_in(dp->base);
	...
	case IXPIO_DIGITAL_OUT:
		...
		digital_out(digital.data.u16, dp->base);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,2,0)
int ixpio821_release(struct inode *inode, struct file *file)
#else
void ixpio821_release(struct inode *inode, struct file *file)
#endif
{
	/* (export)
	 *
...
#endif
	open:ixpio821_open,
	release:ixpio821_release,

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
        unlocked_ioctl:ixpio821_ioctl
#else
	ioctl:ixpio821_ioctl
#endif
};
...
int ixpio821_open(struct inode *inode, struct file *file)
{
	/* (export)
	 *
	 * This function is called by ixpio.o whenever a process attempts to open
	 * the device file of PIO-821
	 *
...
int init_module()
{
	...
	/* align to first PIO-821 in ixpio list */
	for (dev = ixpio_dev; dev && (dev->csid != PIO_821)&&(dev->csid != PIO_821_T3A); dev = dev->next);

        if ((dev->csid != PIO_821)&&(dev->csid != PIO_821_T3A)) {
                printk("fail!\n");
                return FAILURE;
        }
