...
MODULE_DESCRIPTION("ICPDAS PIO-series driver, PISO-P32A32 service module");
...
/*
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
EXPORT_SYMBOL_GPL(ixpisop32a32_ioctl);
EXPORT_SYMBOL_GPL(ixpisop32a32_release);
EXPORT_SYMBOL_GPL(ixpisop32a32_open);
#else
EXPORT_SYMBOL_NOVERS(ixpisop32a32_ioctl);
EXPORT_SYMBOL_NOVERS(ixpisop32a32_release);
EXPORT_SYMBOL_NOVERS(ixpisop32a32_open);
#endif
*/
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
long ixpisop32a32_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#else
int ixpisop32a32_ioctl(struct inode *inode, struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#endif
{
	/* (export)
	 *
...
int ixpisop32a32_release(struct inode *inode, struct file *file)
#else
void ixpisop32a32_release(struct inode *inode, struct file *file)
#endif
{
	/* (export)
	 *
...
# if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
        owner:THIS_MODULE,
#endif
	open:ixpisop32a32_open,
	release:ixpisop32a32_release,

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
        unlocked_ioctl:ixpisop32a32_ioctl
#else
	ioctl:ixpisop32a32_ioctl
#endif
};

int ixpisop32a32_open(struct inode *inode, struct file *file)
{
	/* (export)
	 *
	 * This function is called by ixpio.o whenever a process attempts to open
	 * the device file of PISO-P32A32
	 *
...
int init_module()
{
	...
	/* align to first PISO-P32A32 in ixpio list */
	for (dev = ixpio_dev; dev && (dev->csid != PISO_P32A32)&&(dev->csid != PISO_P32A32_T3A)&&(dev->csid != PISO_P32A32_T3B);
		 dev = dev->next);

	if ((dev->csid != PISO_P32A32)&&(dev->csid != PISO_P32A32_T3A)&&(dev->csid != PISO_P32A32_T3B)) {
		printk("fail!\n");
		return FAILURE;
	}
