...
/* File level history (record changes for this file here.)
   ...
   v 0.4.0  24 Mar 2011 by Golden Wang
     Give support to linux kernel 2.6.37.

   v 0.3.1  24 Jul 2007 by Golden Wang
     Give support to PISO-730 v3.1.

   v 0.3.0  28 Jan 2007 by Reed Lai
     Give support to linux kernel 2.6.x

   v 0.2.0 12 Aug 2003 by Reed Lai
     Complex signal.

   v 0.1.5  3 Jul 2003 by Reed Lai
     Fixed a bug about align_to_minor() that caused the Segmentation fault
     when opening a fake device file.

   v 0.1.4 27 Mar 2003 by Reed Lai
   ...
   v 0.1.3 18 Dec 2002 by Reed Lai
     Abendons the use of dp->ipcr.

   v 0.1.2 26 Nov 2002 by Reed Lai
...
MODULE_DESCRIPTION("ICPDAS PIO-series driver, PISO-730 service module");
...
/*
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
EXPORT_SYMBOL_GPL(ixpiso730_ioctl);
EXPORT_SYMBOL_GPL(ixpiso730_release);
EXPORT_SYMBOL_GPL(ixpiso730_open);
#else
EXPORT_SYMBOL_NOVERS(ixpiso730_ioctl);
EXPORT_SYMBOL_NOVERS(ixpiso730_release);
EXPORT_SYMBOL_NOVERS(ixpiso730_open);
#endif
*/
...
unsigned int eoi(ixpio_devinfo_t * dp)
{
	 ...
	 * interrupt source (INT_CHAN_0/1) from APSR (Aux Pin Status Register,)
	...
	for (mask = 1; mask < INT_MASK_SHIFT_STOP_BIT; mask <<= 1) {
		if (imcr & mask) {
			if (ipcr & mask) {
				/* non-inverse mode */
				if (apsr & mask) {
					dp->is |= mask;	/* interrupt source */
					dp->is_edge |= mask;	/* positive edge */
				}
			} else {
				/* inverse mode */
				if (!(apsr & mask)) {
					dp->is |= mask;	/* interrupt source */
					dp->is_edge &= !mask;	/* negative edge */
				}
			}
		}
	}

	outb(ipcr ^ dp->is, dp->base + IPCR);	/* source for next edge. */
...
#endif
		break;
	case IXPIO_IDIO_L:			/* IDO0 ~ IDO7 */
#if IDIO_L_MASK == 0xff
		outb(((unsigned char) reg->value), base + IDIO_L);
#else
		outb(((unsigned char) reg->value) & IDIO_L_MASK, base + IDIO_L);
#endif
		break;
	case IXPIO_IDIO_H:			/* IDO8 ~ IDO15 */
#if IDIO_H_MASK == 0xff
		outb(((unsigned char) reg->value), base + IDIO_H);
#else
		outb(((unsigned char) reg->value) & IDIO_H_MASK, base + IDIO_H);
#endif
		break;
	case IXPIO_IDIO:			/* IDO0 - IDO15 */
		lbyte = (unsigned char) reg->value;
		hbyte = (unsigned char) (reg->value >> 8);
#if IDIO_L_MASK == 0xff
		outb(lbyte, base + IDIO_L);
#else
		outb(lbyte & IDIO_L_MASK, base + IDIO_L);
#endif
#if IDIO_H_MASK == 0xff
		outb(hbyte, base + IDIO_H);
#else
		outb(hbyte & IDIO_H_MASK, base + IDIO_H);
#endif
		break;
	case IXPIO_DIO_L:			/* DO0 ~ DO7 */
#if DIO_L_MASK == 0xff
		outb(((unsigned char) reg->value), base + DIO_L);
#else
		outb(((unsigned char) reg->value) & DIO_L_MASK, base + DIO_L);
#endif
		break;
	case IXPIO_DIO_H:			/* DO8 ~ DO15 */
#if DIO_H_MASK == 0xff
		outb(((unsigned char) reg->value), base + DIO_H);
#else
		outb(((unsigned char) reg->value) & DIO_H_MASK, base + DIO_H);
#endif
		break;
	case IXPIO_DO:
	case IXPIO_DIO:			/* DO0 - DO15 */
		lbyte = (unsigned char) reg->value;
		hbyte = (unsigned char) (reg->value >> 8);
#if DIO_L_MASK == 0xff
		outb(lbyte, base + DIO_L);
#else
		outb(lbyte & DIO_L_MASK, base + DIO_L);
#endif
#if DIO_H_MASK == 0xff
		outb(hbyte, base + DIO_H);
#else
		outb(hbyte & DIO_H_MASK, base + DIO_H);
#endif
		break;
	default:
		return FAILURE;
...
#endif
		break;
	case IXPIO_IDIO_L:			/* IDI0 ~ IDI7 */
#if IDIO_L_MASK == 0xff
		reg->value = inb(base + IDIO_L);
#else
		reg->value = inb(base + IDIO_L) & IDIO_L_MASK;
#endif
		break;
	case IXPIO_IDIO_H:			/* IDI8 ~ IDI15 */
#if IDIO_H_MASK == 0xff
		reg->value = inb(base + IDIO_H);
#else
		reg->value = inb(base + IDIO_H) & IDIO_H_MASK;
#endif
		break;
	case IXPIO_IDIO:			/* IDI0 - IDI15 */
#if IDIO_H_MASK == 0xff
		hbyte = inb(base + IDIO_H);
#else
		hbyte = inb(base + IDIO_H) & IDIO_H_MASK;
#endif
#if IDIO_L_MASK == 0xff
		lbyte = inb(base + IDIO_L);
#else
		lbyte = inb(base + IDIO_L) & IDIO_L_MASK;
#endif
		reg->value = (hbyte << 8) + lbyte;
		break;
	case IXPIO_DIO_L:			/* DI0 ~ DI7 */
#if DIO_L_MASK == 0xff
		reg->value = inb(base + DIO_L);
#else
		reg->value = inb(base + DIO_L) & DIO_L_MASK;
#endif
		break;
	case IXPIO_DIO_H:			/* DI8 ~ DI15 */
#if DIO_H_MASK == 0xff
		reg->value = inb(base + DIO_H);
#else
		reg->value = inb(base + DIO_H) & DIO_H_MASK;
#endif
		break;
	case IXPIO_DI:
	case IXPIO_DIO:			/* DI0 - DI15 */
#if DIO_H_MASK == 0xff
		hbyte = inb(base + DIO_H);
#else
		hbyte = inb(base + DIO_H) & DIO_H_MASK;
#endif
#if DIO_L_MASK == 0xff
		lbyte = inb(base + DIO_L);
#else
		lbyte = inb(base + DIO_L) & DIO_L_MASK;
#endif
		reg->value = (hbyte << 8) + lbyte;
		break;
	default:
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
long ixpiso730_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#else
int ixpiso730_ioctl(struct inode *inode, struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#endif
{
	/* (export)
	 *
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,2,0)
int ixpiso730_release(struct inode *inode, struct file *file)
#else
void ixpiso730_release(struct inode *inode, struct file *file)
#endif
{
	/* (export)
	 *
...
#endif
	open:ixpiso730_open,
	release:ixpiso730_release,

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
        unlocked_ioctl:ixpiso730_ioctl
#else
	ioctl:ixpiso730_ioctl
#endif
};
...
int ixpiso730_open(struct inode *inode, struct file *file)
{
	/* (export)
	 *
	 * This function is called by ixpio.o whenever a process attempts to open
	 * the device file of PISO-730
	 *
...
int init_module()
{
	...
	/* align to first PISO-730 in ixpio list */
	for (dev = ixpio_dev; dev && (dev->csid != PISO_730)&&(dev->csid != PISO_730_T3A)&&(dev->csid != PISO_730_T3B)&&(dev->csid != PISO_730_T3C); dev = dev->next);

	if ((dev->csid != PISO_730)&&(dev->csid != PISO_730_T3A)&&(dev->csid != PISO_730_T3B)&&(dev->csid != PISO_730_T3C)) {
		printk("fail!\n");
		return FAILURE;
	}
