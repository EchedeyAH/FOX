...
/* File level history (record changes for this file here.)
   ...
   v 0.2.3 18 Dec 2002 by Reed Lai
     Abendons the use of dp->ipcr.

   v 0.2.2 26 Nov 2002 by Reed Lai
     Corrects the BOTTOM_HELF condition (#if, #ifdef)

   v 0.2.1  7 Nov 2002 by Reed Lai
   ...
   v 0.2.0  4 Jun 2002 by Reed Lai
     Conditionally remove the bottom helf of the interrupt handler.
     Refine fome codes.

   v 0.1.0 29 Apr 2002 by Reed Lai
     Remove unused declarations "msg" and "msg_ptr."
     Rename PIO to IXPIO.

   v 0.0.1 8 May 2001 by Reed Lai
     Correct the removing family fop of cleanup_module by "next" to "next_f"
     This bug will cause the cleanup_module to remove fop that does not belong
     to it.

   v 0.0.0 2000.11.07 by Reed Lai
     create, blah blah... */
...
MODULE_DESCRIPTION("ICPDAS PIO-series driver, PIO-D48 service module");
...
/*
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
EXPORT_SYMBOL_GPL(ixpiod48_ioctl);
EXPORT_SYMBOL_GPL(ixpiod48_release);
EXPORT_SYMBOL_GPL(ixpiod48_open);
#else
EXPORT_SYMBOL_NOVERS(ixpiod48_ioctl);
EXPORT_SYMBOL_NOVERS(ixpiod48_release);
EXPORT_SYMBOL_NOVERS(ixpiod48_open);
#endif
*/
...
unsigned int eoi(ixpio_devinfo_t * dp)
{
	...
	unsigned int ipcr;			/* Interrupt Polarity Control Register */
	...
	//printk("apsr is %d imcr is %d ipcr is %d\n",apsr,imcr,ipcr);	//winson
...
#endif
		break;
	case IXPIO_82551PA:		/* 8255-1 Port A */
#if RW82551PA_MASK == 0xff
		outb(((unsigned char) reg->value), base + RW82551PA);
#else
		outb(((unsigned char) reg->value) & RW82551PA_MASK,
			 base + RW82551PA);
#endif
		break;
	case IXPIO_82551PB:		/* 8255-1 Port B */
#if RW82551PB_MASK == 0xff
		outb(((unsigned char) reg->value), base + RW82551PB);
#else
		outb(((unsigned char) reg->value) & RW82551PB_MASK,
			 base + RW82551PB);
#endif
	//printk(KERN_INFO FAMILY ": PB 0x%x 0x%x\n", RW82551PB, reg->value);
		break;
	case IXPIO_82551PC:		/* 8255-1 Port C */
#if RW82551PC_MASK == 0xff
		outb(((unsigned char) reg->value), base + RW82551PC);
#else
		outb(((unsigned char) reg->value) & RW82551PC_MASK,
			 base + RW82551PC);
#endif
		break;
	case IXPIO_82551CW:		/* 8255-1 Control Word */
#if RW82551CW_MASK == 0xff
		outb(((unsigned char) reg->value), base + RW82551CW);
#else
		outb(((unsigned char) reg->value) & RW82551CW_MASK,
			 base + RW82551CW);
#endif
	//printk(KERN_INFO FAMILY ": CW 0x%x 0x%x\n", RW82551CW, reg->value);
		break;
	case IXPIO_82552PA:		/* 8255-2 Port A */
#if RW82552PA_MASK == 0xff
		outb(((unsigned char) reg->value), base + RW82552PA);
#else
		outb(((unsigned char) reg->value) & RW82552PA_MASK,
			 base + RW82552PA);
#endif
		break;
	case IXPIO_82552PB:		/* 8255-2 Port B */
#if RW82552PB_MASK == 0xff
		outb(((unsigned char) reg->value), base + RW82552PB);
#else
		outb(((unsigned char) reg->value) & RW82552PB_MASK,
			 base + RW82552PB);
#endif
		break;
	case IXPIO_82552PC:		/* 8255-2 Port C */
#if RW82552PC_MASK == 0xff
		outb(((unsigned char) reg->value), base + RW82552PC);
#else
		outb(((unsigned char) reg->value) & RW82552PC_MASK,
			 base + RW82552PC);
#endif
		break;
	case IXPIO_82552CW:		/* 8255-2 Control Word */
#if RW82552CW_MASK == 0xff
		outb(((unsigned char) reg->value), base + RW82552CW);
#else
		outb(((unsigned char) reg->value) & RW82552CW_MASK,
			 base + RW82552CW);
#endif
		break;
	case IXPIO_8254C0:			/* 8254 Counter 0 */
#if RW8254C0_MASK == 0xff
		outb(((unsigned char) reg->value), base + RW8254C0);
#else
		outb(((unsigned char) reg->value) & RW8254C0_MASK,
			 base + RW8254C0);
#endif
		break;
	case IXPIO_8254C1:			/* 8254 Counter 1 */
#if RW8254C1_MASK == 0xff
		outb(((unsigned char) reg->value), base + RW8254C1);
#else
		outb(((unsigned char) reg->value) & RW8254C1_MASK,
			 base + RW8254C1);
#endif
		break;
	case IXPIO_8254C2:			/* 8254 Counter 2 */
#if RW8254C2_MASK == 0xff
		outb(((unsigned char) reg->value), base + RW8254C2);
#else
		outb(((unsigned char) reg->value) & RW8254C2_MASK,
			 base + RW8254C2);
#endif
		break;
	case IXPIO_8254CW:			/* 8254 Control Word */
#if RW8254CW_MASK == 0xff
		outb(((unsigned char) reg->value), base + RW8254CW);
#else
		outb(((unsigned char) reg->value) & RW8254CW_MASK,
			 base + RW8254CW);
#endif
		break;
	case IXPIO_CICR:			/* Clock/Int Control Register */
#if CICR_MASK == 0xff
		outb(((unsigned char) reg->value), base + CICR);
#else
		outb(((unsigned char) reg->value) & CICR_MASK, base + CICR);
#endif
		break;
	default:
		return FAILURE;
...
int read_reg(ixpio_reg_t * reg, unsigned long int base)
{
	...
	case IXPIO_82551PA:		/* 8255-1 Port A */
#if RW82551PA_MASK == 0xff
		reg->value = inb(base + RW82551PA);
#else
		reg->value = inb(base + RW82551PA) & RW82551PA_MASK;
#endif
		break;
	case IXPIO_82551PB:		/* 8255-1 Port B */
#if RW82551PB_MASK == 0xff
		reg->value = inb(base + RW82551PB);
#else
		reg->value = inb(base + RW82551PB) & RW82551PB_MASK;
#endif
		break;
	case IXPIO_82551PC:		/* 8255-1 Port C */
#if RW82551PC_MASK == 0xff
		reg->value = inb(base + RW82551PC);
#else
		reg->value = inb(base + RW82551PC) & RW82551PC_MASK;
#endif
		break;
	case IXPIO_82551CW:		/* 8255-1 Control Word */
#if RW82551CW_MASK == 0xff
		reg->value = inb(base + RW82551CW);
#else
		reg->value = inb(base + RW82551CW) & RW82551CW_MASK;
#endif
		break;
	case IXPIO_82552PA:		/* 8255-2 Port A */
#if RW82552PA_MASK == 0xff
		reg->value = inb(base + RW82552PA);
#else
		reg->value = inb(base + RW82552PA) & RW82552PA_MASK;
#endif
		break;
	case IXPIO_82552PB:		/* 8255-2 Port B */
#if RW82552PB_MASK == 0xff
		reg->value = inb(base + RW82552PB);
#else
		reg->value = inb(base + RW82552PB) & RW82552PB_MASK;
#endif
		break;
	case IXPIO_82552PC:		/* 8255-2 Port C */
#if RW82552PC_MASK == 0xff
		reg->value = inb(base + RW82552PC);
#else
		reg->value = inb(base + RW82552PC) & RW82552PC_MASK;
#endif
		break;
	case IXPIO_82552CW:		/* 8255-2 Control Word */
#if RW82552CW_MASK == 0xff
		reg->value = inb(base + RW82552CW);
#else
		reg->value = inb(base + RW82552CW) & RW82552CW_MASK;
#endif
		break;
	case IXPIO_8254C0:			/* 8254 Counter 0 */
#if RW8254C0_MASK == 0xff
		reg->value = inb(base + RW8254C0);
#else
		reg->value = inb(base + RW8254C0) & RW8254C0_MASK;
#endif
		break;
	case IXPIO_8254C1:			/* 8254 Counter 1 */
#if RW8254C1_MASK == 0xff
		reg->value = inb(base + RW8254C1);
#else
		reg->value = inb(base + RW8254C1) & RW8254C1_MASK;
#endif
		break;
	case IXPIO_8254C2:			/* 8254 Counter 2 */
#if RW8254C2_MASK == 0xff
		reg->value = inb(base + RW8254C2);
#else
		reg->value = inb(base + RW8254C2) & RW8254C2_MASK;
#endif
		break;
	case IXPIO_8254CW:			/* 8254 Control Word */
#if RW8254CW_MASK == 0xff
		reg->value = inb(base + RW8254CW);
#else
		reg->value = inb(base + RW8254CW) & RW8254CW_MASK;
#endif
		break;
	case IXPIO_CICR:			/* Clock/Int Control Register */
#if CICR_MASK == 0xff
		reg->value = inb(base + CICR);
#else
		reg->value = inb(base + CICR) & CICR;
#endif
		break;
	case IXPIO_CID:
#if CID_MASK == 0xff
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
long ixpiod48_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#else
int ixpiod48_ioctl(struct inode *inode, struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#endif
{
	/* (export)
	 *
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,2,0)
int ixpiod48_release(struct inode *inode, struct file *file)
#else
void ixpiod48_release(struct inode *inode, struct file *file)
#endif
{
	/* (export)
	 *
...
#endif
	open:ixpiod48_open,
	release:ixpiod48_release,

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
        unlocked_ioctl:ixpiod48_ioctl
#else
	ioctl:ixpiod48_ioctl
#endif
};
...
int ixpiod48_open(struct inode *inode, struct file *file)
{
	/* (export)
	 *
	 * This function is called by ixpio.o whenever a process attempts to open
	 * the device file of PIO-D48
	 *
...
int init_module()
{
	...
	/* align to first PIO-D48 in ixpio list */
	for (dev = ixpio_dev; dev && (dev->csid != PIO_D48)&&(dev->csid != PIO_D48_T3A)&&(dev->csid != PIO_D48_T3B); dev = dev->next);

	if ((dev->csid != PIO_D48)&&(dev->csid != PIO_D48_T3A)&&(dev->csid != PIO_D48_T3B)) {
		printk("fail!\n");
		return FAILURE;
	}
