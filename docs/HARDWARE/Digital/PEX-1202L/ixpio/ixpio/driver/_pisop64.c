...
MODULE_DESCRIPTION("ICPDAS PIO-series driver, PISO-P64 service module");
...
/*
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
EXPORT_SYMBOL_GPL(ixpisop64_ioctl);
EXPORT_SYMBOL_GPL(ixpisop64_release);
EXPORT_SYMBOL_GPL(ixpisop64_open);
#else
EXPORT_SYMBOL_NOVERS(ixpisop64_ioctl);
EXPORT_SYMBOL_NOVERS(ixpisop64_release);
EXPORT_SYMBOL_NOVERS(ixpisop64_open);
#endif
*/
...
#endif
		break;
	case IXPIO_DI_A:			/* DI0 ~ DI7 */
#if DI_A_MASK == 0xff
		reg->value = inb(base + DI_A);
#else
		reg->value = inb(base + DI_A) & DI_A_MASK;
#endif
		break;
	case IXPIO_DI_B:			/* DI8 ~ DI15 */
#if DI_B_MASK == 0xff
		reg->value = inb(base + DI_B);
#else
		reg->value = inb(base + DI_B) & DI_B_MASK;
#endif
		break;
	case IXPIO_DI_C:			/* DI16 ~ DI23 */
#if DI_C_MASK == 0xff
		reg->value = inb(base + DI_C);
#else
		reg->value = inb(base + DI_C) & DI_C_MASK;
#endif
		break;
	case IXPIO_DI_D:			/* DI24 ~ DI31 */
#if DI_D_MASK == 0xff
		reg->value = inb(base + DI_D);
#else
		reg->value = inb(base + DI_D) & DI_D_MASK;
#endif
		break;
	case IXPIO_DI_E:			/* DI32 ~ DI39 */
#if DI_E_MASK == 0xff
		reg->value = inb(base + DI_E);
#else
		reg->value = inb(base + DI_E) & DI_E_MASK;
#endif
		break;
	case IXPIO_DI_F:			/* DI40 ~ DI47 */
#if DI_F_MASK == 0xff
		reg->value = inb(base + DI_F);
#else
		reg->value = inb(base + DI_F) & DI_F_MASK;
#endif
		break;
	case IXPIO_DI_G:			/* DI48 ~ DI55 */
#if DI_G_MASK == 0xff
		reg->value = inb(base + DI_G);
#else
		reg->value = inb(base + DI_G) & DI_G_MASK;
#endif
		break;
	case IXPIO_DI_H:			/* DI56 ~ DI63 */
#if DI_H_MASK == 0xff
		reg->value = inb(base + DI_H);
#else
		reg->value = inb(base + DI_H) & DI_H_MASK;
#endif
		break;
	case IXPIO_DI:
	default:
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
long ixpisop64_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#else
int ixpisop64_ioctl(struct inode *inode, struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#endif
{
	/* (export)
	 *
...
int ixpisop64_release(struct inode *inode, struct file *file)
#else
void ixpisop64_release(struct inode *inode, struct file *file)
#endif
{
	/* (export)
	 *
...
#endif
	open:ixpisop64_open,
	release:ixpisop64_release,

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
        unlocked_ioctl:ixpisop64_ioctl
#else
	ioctl:ixpisop64_ioctl
#endif
};

int ixpisop64_open(struct inode *inode, struct file *file)
{
	/* (export)
	 *
	 * This function is called by ixpio.o whenever a process attempts to open
	 * the device file of PISO-P64
	 *
...
int init_module()
{
	...
	/* align to first PISO-P64 in ixpio list */
	for (dev = ixpio_dev; dev && (dev->csid != PISO_P64)&&(dev->csid != PISO_P64_T3A)&&(dev->csid != PISO_P64_T3B);
		 dev = dev->next);

	if ((dev->csid != PISO_P64)&&(dev->csid != PISO_P64_T3A)&&(dev->csid != PISO_P64_T3B)) {
		printk("fail!\n");
		return FAILURE;
	}
