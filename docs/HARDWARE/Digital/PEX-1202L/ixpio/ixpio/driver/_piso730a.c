...
/* File level history (record changes for this file here.)
   ...
   v 0.0.4 27 Mar 2003 by Reed Lai
     Fixec the bug that free_irq with different argument to the request_irq.
   ...
   v 0.0.1  7 Nov 2002 by Reed Lai
...
MODULE_DESCRIPTION("ICPDAS PIO-series driver, PISO-730A service module");
...
/*
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
EXPORT_SYMBOL_GPL(ixpiso730a_ioctl);
EXPORT_SYMBOL_GPL(ixpiso730a_release);
EXPORT_SYMBOL_GPL(ixpiso730a_open);
#else
EXPORT_SYMBOL_NOVERS(ixpiso730a_ioctl);
EXPORT_SYMBOL_NOVERS(ixpiso730a_release);
EXPORT_SYMBOL_NOVERS(ixpiso730a_open);
#endif
*/
...
unsigned int eoi(ixpio_devinfo_t * dp)
{
	 ...
	 * interrupt source (P2C0/1/2/3) from APSR (Aux Pin Status Register,)
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
long ixpiso730a_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#else
int ixpiso730a_ioctl(struct inode *inode, struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#endif
{
	/* (export)
	 *
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,2,0)
int ixpiso730a_release(struct inode *inode, struct file *file)
#else
void ixpiso730a_release(struct inode *inode, struct file *file)
#endif
{
	/* (export)
	 *
...
# if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
        owner:THIS_MODULE,
#endif
	open:ixpiso730a_open,
	release:ixpiso730a_release,

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
        unlocked_ioctl:ixpiso730a_ioctl
#else
	ioctl:ixpiso730a_ioctl
#endif
};
...
int ixpiso730a_open(struct inode *inode, struct file *file)
{
	/* (export)
	 *
	 * This function is called by ixpio.o whenever a process attempts to open
	 * the device file of PISO-730A
	 *
...
int init_module()
{
	...
	/* align to first PISO-730A in ixpio list */
	for (dev = ixpio_dev; dev && (dev->csid != PISO_730A)&&(dev->csid != PISO_730A_T3A)&&(dev->csid != PISO_730A_T3B); dev = dev->next);

	if ((dev->csid != PISO_730A)&&(dev->csid != PISO_730A_T3A)&&(PISO_730A_T3B)) {
		printk("fail!\n");
		return FAILURE;
	}
