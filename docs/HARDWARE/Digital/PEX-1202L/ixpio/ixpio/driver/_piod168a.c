...
/* File level history (record changes for this file here.)
   ...
   v 0.3.0  8 Aug 2003 by Reed Lai
     Complex signal condition.

   v 0.2.6  9 Jul 2003 by Reed Lai
   ...
   v 0.2.2 18 Nov 2002 by Reed Lai
     Comments out the including of the slab.h which is no longer used
     by this module.
   ...
   v 0.2.1 26 Nov 2002 by Reed Lai
   ...
   v 0.1.0 29 Apr 2002 by Reed Lai
     Remove unused declaration "msg" and "msg_ptr."
     Rename PIO to IXPIO.
...
MODULE_DESCRIPTION("ICPDAS PIO-series driver, PIO-D168A service module");
...
/*
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
EXPORT_SYMBOL_GPL(ixpiod168a_ioctl);
EXPORT_SYMBOL_GPL(ixpiod168a_release);
EXPORT_SYMBOL_GPL(ixpiod168a_open);
#else
EXPORT_SYMBOL_NOVERS(ixpiod168a_ioctl);
EXPORT_SYMBOL_NOVERS(ixpiod168a_release);
EXPORT_SYMBOL_NOVERS(ixpiod168a_open);
#endif
*/
...
#endif
		break;
	case IXPIO_8DR:			/* Read/Write 8bit Data Register */
#if _8DR_MASK == 0xff
		outb(((unsigned char) reg->value), base + _8DR);
#else
		outb(((unsigned char) reg->value) & _8DR_MASK, base + _8DR);
#endif
		break;
	case IXPIO_AIOPCR:			/* Active IO Port Control Register */
#if AIOPCR_MASK == 0xff
		outb(((unsigned char) reg->value), base + AIOPCR);
#else
		outb(((unsigned char) reg->value) & AIOPCR_MASK, base + AIOPCR);
#endif
		break;
	case IXPIO_IOSCRA:			/* IO Selection Control Register A */
#if IOSCRA_MASK == 0xff
		outb(((unsigned char) reg->value), base + IOSCRA);
#else
		outb(((unsigned char) reg->value) & IOSCRA_MASK, base + IOSCRA);
#endif
		break;
	case IXPIO_IOSCRB:			/* IO Selection Control Register B */
#if IOSCRB_MASK == 0xff
		outb(((unsigned char) reg->value), base + IOSCRB);
#else
		outb(((unsigned char) reg->value) & IOSCRB_MASK, base + IOSCRB);
#endif
		break;
	case IXPIO_IOSCRC:			/* IO Selection Control Register C */
#if IOSCRC_MASK == 0xff
		outb(((unsigned char) reg->value), base + IOSCRC);
#else
		outb(((unsigned char) reg->value) & IOSCRC_MASK, base + IOSCRC);
#endif
		break;
	case IXPIO_IOSCRD:			/* IO Selection Control Register D */
#if IOSCRD_MASK == 0xff
		outb(((unsigned char) reg->value), base + IOSCRD);
#else
		outb(((unsigned char) reg->value) & IOSCRD_MASK, base + IOSCRD);
#endif
		break;
	default:
		return FAILURE;
...
int read_reg(ixpio_reg_t * reg, unsigned long int base)
{
	...
	case IXPIO_8DR:			/* Read/Write 8bit Data Register */
#if _8DR_MASK == 0xff
		reg->value = inb(base + _8DR);
#else
		reg->value = inb(base + _8DR) & _8DR_MASK;
#endif
		break;
	case IXPIO_AIOPCR:			/* Active IO Port Control Register */
#if AIOPCR_MASK == 0xff
		reg->value = inb(base + AIOPCR);
#else
		reg->value = inb(base + AIOPCR) & AIOPCR_MASK;
#endif
		break;
	default:
		return FAILURE;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
long ixpiod168a_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#else
int ixpiod168a_ioctl(struct inode *inode, struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#endif
{
	/* (export)
	 *
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,2,0)
int ixpiod168a_release(struct inode *inode, struct file *file)
#else
void ixpiod168a_release(struct inode *inode, struct file *file)
#endif
{
	/* (export)
	 *
...
# if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
        owner:THIS_MODULE,
#endif
	open:ixpiod168a_open,
	release:ixpiod168a_release,

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
	unlocked_ioctl:ixpiod168a_ioctl
#else
	ioctl:ixpiod168a_ioctl
#endif
};
...
int ixpiod168a_open(struct inode *inode, struct file *file)
{
	/* (export)
	 *
	 * This function is called by ixpio.o whenever a process attempts to open
	 * the device file of PIO-D168A
	 *
...
int init_module()
{
	...
	/* align to first PIO-D168A in ixpio list */
	for (dev = ixpio_dev; dev && (dev->csid != PIO_D168A)&&(dev->csid != PIO_D168_T3A)&&(dev->csid != PIO_D168_T3B); dev = dev->next);

	if ((dev->csid != PIO_D168A)&&(dev->csid != PIO_D168_T3A)&&(PIO_D168_T3B)) {
		printk("fail!\n");
		return FAILURE;
	}
