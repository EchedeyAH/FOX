...
/*
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
EXPORT_SYMBOL_GPL(ixpiod64_ioctl);
EXPORT_SYMBOL_GPL(ixpiod64_release);
EXPORT_SYMBOL_GPL(ixpiod64_open);
#else
EXPORT_SYMBOL_NOVERS(ixpiod64_ioctl);
EXPORT_SYMBOL_NOVERS(ixpiod64_release);
EXPORT_SYMBOL_NOVERS(ixpiod64_open);
#endif
*/
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
long ixpiod64_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#else
int ixpiod64_ioctl(struct inode *inode, struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
#endif
{
	/* (export)
	 *
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,2,0)
int ixpiod64_release(struct inode *inode, struct file *file)
#else
void ixpiod64_release(struct inode *inode, struct file *file)
#endif
{
	/* (export)
	 *
...
#endif
	open:ixpiod64_open,
	release:ixpiod64_release,
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
        unlocked_ioctl:ixpiod64_ioctl
#else
	ioctl:ixpiod64_ioctl
#endif
};
...
int ixpiod64_open(struct inode *inode, struct file *file)
{
	/* (export)
	 *
...
int init_module()
{
	...
	for (dev = ixpio_dev; dev && (dev->csid != PIO_D64)&&(dev->csid != PIO_D64_T3A)&&(dev->csid != PIO_D64_T3B); dev = dev->next);

	if ((dev->csid != PIO_D64 )&&(dev->csid != PIO_D64_T3A)&&(dev->csid != PIO_D64_T3B))  {
		printk("fail!\n");
		return FAILURE;
	}
	...
	for (dp = dev; dp; dp = dp->next_f) {
		...
		request_region(dp->base, IO_RANGE, MODULE_NAME);
		digital_out(0, dp->base);			//Set DO 0
		outb(BOARD_DIO_DISABLE, dp->base + RCR);	//Set DO off
		release_region(dp->base, IO_RANGE);
